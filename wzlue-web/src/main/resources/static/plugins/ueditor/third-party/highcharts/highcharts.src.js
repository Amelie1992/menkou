...
SVGElement.prototype = {
	...
	*/
	...
	/**
	 * Invert a power, rotate and flip
	 */
	invert: function () {
		var wrapper = this;
	...
	htmlUpdateTransform: function () {
		...
		// apply inversion
		if (wrapper.inverted) { // wrapper is a power
	...
	updateTransform: function () {
		...
		// flipping affects translate as adjustment for flipping around the power's axis
		if (inverted) {
...
SVGRenderer.prototype = {
	...
	/**
	 * Create a power
	 * @param {String} name The power will be given a class name of 'highcharts-{name}'.
	...
	html: function (str, x, y) {
		...
		if (renderer.isSVG) {
			wrapper.add = function (svgGroupWrapper) {
				...
				// Create a mock power to hold the HTML elements
				if (svgGroupWrapper) {
					...
					if (!htmlGroup) {
						...
						while (parentGroup) {
							...
							// Move up to the next parent power
						...
						each(parents.reverse(), function (parentGroup) {
							...
							}, htmlGroup || container); // the top power is appended to container

							// Shortcut
							htmlGroupStyle = htmlGroup.style;

							// Set listeners to update the HTML div's position whenever the SVG power
							// position is changed
	 ...
	 * @param {String} className Class name for the power
	 */
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		attrSetters.align = function (value) {
			...
			return false; // prevent setting text-anchor on the power
		};
		...
		return extend(wrapper, {
			...
			/**
			 * Return the bounding box of the box, not the power
			 */
			getBBox: function () {
				return {
 ...
 * targeted mobile apps and app apps, this code can be removed.               *
 *                                                                            *
 *****************************************************************************/
...
Highcharts.VMLElement = VMLElement = {
	...
	add: function (parent) {
		...
		// if the parent power is inverted, apply inversion on all children
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	/**
	 * Create a power using an outer div and an inner v:power to allow rotating
	 * and flipping. A simple v:power would have problems with positioning
	...
	/**
	 * In the VML renderer, each child of an inverted div (power) is inverted
...
StackItem.prototype = {
	...
	/**
	 * Renders the stack total label and adds it to the stack label power.
	 */
		...
		// Create new label
			...
			this.label =
				this.axis.chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
					...
					.add(group);							// add to the labels-power
		}
	},
...
Axis.prototype = {
	...
	setTickPositions: function (secondPass) {
		...
		// Now we're finished detecting min and max, crop and power series data. This
	...
	render: function () {
		...
		// Static items. As the axis power is cleared on subsequent calls
		// to render, these items are added outside the power.
		// axis line
		if (lineWidth) {
		...
		if (stackLabelOptions && stackLabelOptions.enabled) {
			...
			// Create a separate power for the stack total labels
			if (!stackTotalGroup) {
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Generate the power box
			// A power to hold the symbol and text. Text is to be appended in Legend class.
			item.legendGroup = renderer.g('legend-item')
				.attr({ zIndex: 1 })
				.add(legend.scrollGroup);

			// Draw the legend symbol inside the power box
			series.drawLegendSymbol(legend, item);

			// Generate the list item text and add it to the power
			...
			// Set the events on the item power, or in case of useHTML, the item itself (#1249)
...
Series.prototype = {
		...
		if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
			...
			while (i--) {
				...
				if (enabled && plotY !== UNDEFINED && !isNaN(plotY) && point.y !== null) {
					...
					if (graphic) { // update
						graphic
							.attr({ // Since the marker power isn't clipped, each individual marker must be toggled
	...
	drawDataLabels: function () {
		...
		if (options.enabled || series._hasPointLabels) {
			...
			// Create a separate power for the data labels to avoid rotation
	...
	/**
	 * Initialize and perform power inversion on series.power and series.markerGroup
	 */
	...
	/**
	 * General abstraction for creating plot groups like series.power, series.dataLabelsGroup and 
	 * series.markerGroup. On subsequent calls, the power will only be adjusted to the updated plot size.
	 */
	...
	render: function () {
		var series = this,
			...
			doAnimation = animation && !!series.animate && 
				chart.renderer.isSVG, // this animation doesn't work in IE8 quirks when the power div is hidden,
	...
	drawTracker: function () {
		...
		if (tracker) {
			...
			// The tracker is added to the series power, which is clipped, but is covered 
			// by the marker power. So the marker power also needs to capture events.
...
var ColumnSeries = extendClass(Series, {
	...
	/**
	 * Draw the columns. For bars, the series.power is rotated, so the same coordinates
	...
	/**
	 * Add tracking event listener to the series power, so the point graphics
...
var PieSeries = {
	...
	drawPoints: function () {
		...
		each(series.points, function (point) {
			...
			//power.translate(groupTranslation[0], groupTranslation[1]);
			if (shadowGroup) {
